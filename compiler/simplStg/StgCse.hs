...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The second kind of CSE opportunity we aim for is more interesting.
The Haskell code
...
data CseEnv = CseEnv
    { ce_conAppMap :: ConAppMap Id
        ...
        --   data constructor applications to an in-scope ID that can be used
        --   instead.
    , ce_subst     :: IdEnv Id
    ...
    , ce_in_scope  :: InScopeSet
        -- ^ The third component is an in-scope set, to rename away any binders
        --   that might shadow an id mentioned in the other two components.
    }
...
addEnv _ _ [] env = env
addEnv bndr dataCon args env
    = env { ce_conAppMap = new_env, ce_in_scope = new_in_scope}
  where
    new_env = insertTM (dataCon, args) bndr (ce_conAppMap env)
    new_in_scope = extendInScopeSetList (ce_in_scope env) $
...
addSubst from to env
    = env { ce_subst = extendVarEnv (ce_subst env) from to
          , ce_in_scope = ce_in_scope env `extendInScopeSet` from `extendInScopeSet` to
          }
...
substVar env from
    | Just to <- lookupVarEnv (ce_subst env) from = to
    | otherwise                                   = from
